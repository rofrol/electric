...
(defmacro button [V! & body]
  `(dom/button
     (let [ret# (do ~@body)
       ...
       (case state#
         ::e/idle    nil
         ::e/pending (dom/>>style :background-color "yellow")
         ::e/ok      (keep-for 1000 (dom/>>style :border-color "green"))
         ;; TODO tooltip v# on hover or something smarter than `prn`
         ::e/failed  (do (prn ::button :err v#) (keep-for 1000 (dom/>>style :border-color "red"))))
       ret#)))
