(ns wip.reagent-interop
  ...
  #?(:cljs (:require-macros wip.reagent-interop)))      ; forces shadow hot reload to also reload JVM at the same time
...
(def react-root-hook "See `p/with`"
  #?(:clj  dom/unsupported
     :cljs (fn ([x] (prn "unmount") (.unmount x))    ; unmount
             ([x y] (.insertBefore (.-parentNode x) x y)) ; TODO rotate siblings
             )))
...
(defmacro with-reagent [& args]
  `(dom/div  ; React will hijack this element and empty it.
     (binding [dom/node (create-root dom/node)]
       (new (p/hook react-root-hook dom/node
              (p/fn [] dom/keepalive
...
(p/defn App []
  (dom/div {:style {:width "100vw", :height "100vh"}}
    (dom/h1 (dom/text "Reagent/React Interop"))
    (let [event (dom/events js/document "mousemove" nil nil nil)]
      ...
      (with-reagent TinyLineChart [{:name "Page A", :uv 4000, :pv 2400, :amt 2400}
                                   ...
                                   {:name "Page C", :uv 2000, :pv (* 2 (:clientY event 1000)), :amt 2290}  ; inject value
                                   {:name "Page D", :uv 2780, :pv 3908, :amt 2000}
                                   {:name "Page E", :uv 1890, :pv 4800, :amt 2181}
                                   {:name "Page F", :uv 2390, :pv 3800, :amt 2500}
                                   {:name "Page G", :uv 3490, :pv 4300, :amt 2100}])
      (with-reagent MousePosition (:clientX event 0) (:clientY event 0)))))
